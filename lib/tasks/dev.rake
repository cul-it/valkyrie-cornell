...
if Rails.env.development? || Rails.env.test?
  ...
  namespace :server do
    ...
    task :clean_test do
      SolrWrapper.instance(managed: true, verbose: true, port: 8984, instance_dir: 'tmp/blacklight-core-test', persist: true).remove_instance_dir!
      SolrWrapper.instance(managed: true, verbose: true, port: 8985, instance_dir: 'tmp/hydra-test', persist: true).remove_instance_dir!
    ...
    task :development do
      SolrWrapper.wrap(managed: true, verbose: true, port: 8983, instance_dir: 'tmp/blacklight-core', persist: true) do |solr|
        solr.with_collection(name: "blacklight-core", dir: Rails.root.join("solr", "config").to_s) do
          SolrWrapper.wrap(managed: true, verbose: true, port: 8987, instance_dir: 'tmp/hydra-dev', persist: true) do |solr2|
